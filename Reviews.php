...
$rev = $db->query("SELECT * FROM `reviews`")->fetchAll();
...
if(isset($_POST)){
    ...
    if(!empty($_POST['revForm'])){
        $name = $_POST['name'];
        $rev = $_POST['rev'];
        // echo("<script>alert('Ошибка решена!');</script>");

        if(empty($_SESSION['user'])){        
            $query = $db->query("
            INSERT INTO `reviews` 
            (`id`, `iduser`, `name`, `reviews`) VALUES 
            (NULL, NULL, '$name', '$rev')
            ");
        }else{
            ...
            INSERT INTO `reviews` 
            (`id`, `iduser`, `name`, `reviews`) VALUES 
            (NULL, '$iduser', '$name', '$rev')
            ");
        }

        if($query){
            echo("
            <script>alert('Отзыв отправлен в базу данных!');location.href = 'Reviews.php'</script>
            ");
        }else{
            echo "<script>alert('У вас ошибка!')</script>";
            print_r($db->errorInfo());
...
<body>
   ...
   <div class="darkness">
    ...
    <main>
        ...
        <div class="reviuvs">
            <div class="rev">
                <img src="image/LogINeed.png" alt="">
                <p class="accent">Тачибана Ариса</p>
                <p>Обучение хорошее, но было б лушче, если бы не злой охранник</p>
            </div>
            <div class="rev">
                <img src="image/UnLoggined.png" alt="">
                <p class="accent">Аноним</p>
                <p>10 из 10</p>
            </div>
            <div class="rev">
                <img src="image/UnLoggined.png" alt="">
                <p class="accent">Неизвестный</p>
                <p>Всё плохо</p>
            </div>
            <div class="rev">
                <img src="image/UnLoggined.png" alt="">
                <p class="accent">Анон</p>
                <p>Обучение здесь зачёт</p>
            </div>

            <?php foreach($rev as $re){ ?>
            <div class="rev">
            <img src="image/UnLoggined.png" alt="">
            <!-- Тут должна была быть приколямба с получение картинки из профиля юзера, но мне сейчас лень -->
            <p class="accent"><?php print($re['name']); ?></p>
            <p><?php print($re['reviews']); ?></p>
            </div>
        ...
        <h2>Форма на отзывы</h2>
        <form class="revForm" method='post'>
            <input type="hidden" name="revForm" value='12' >
            <label>Имя</label>
            <input type="text" name='name' id="Names" value="<?php print($_SESSION['user']['name']); ?>">
            <label>Отзыв</label>
            <textarea rows="5" name='rev' id="Reviu" placeholder='Ваш отзыв'></textarea>
